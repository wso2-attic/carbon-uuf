...
public class HbsPageRenderable extends HbsRenderable {
    ...
    public String render(Model model, Lookup lookup, RequestLookup requestLookup, API api) {
        Context context;
        if (executable == null) {
            context = Context.newContext(getHbsModel(lookup, requestLookup, api));
        } else {
            ...
            context = Context.newContext(executableOutput).combine(getHbsModel(lookup, requestLookup, api));
        }
