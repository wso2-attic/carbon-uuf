...
(function () {
    ...
    addPageUnitDefinitions = function (unitModels, dir) {
        var siteFiles = new File(dir || '/site').listFiles();
        for (var i = 0; i < siteFiles.length; i++) {
            var siteFile = siteFiles[i];
            var fileName = siteFile.getName();
            if (siteFile.isDirectory()) {
                addPageUnitDefinitions(unitModels, siteFile.getPath())
            } else if (fileName.indexOf('.hbs', fileName.length - 4) !== -1) { // File name ends with '.hbs'

                //path relative to app root
                var relativePath = siteFile.getPath()
                    .substring(5 + siteFile.getPath().indexOf('/site/'), siteFile.getPath().length - 4);
