...
(function () {
    ...
    route = function () {
        ...
        if (parts.head == 'debug') { // {one} == 'debug'
            var debuggerLibFile = new File('/debug' + parts.tail + '.jag');
            if (debuggerLibFile.isExists()) {
                include(debuggerLibFile.getPath());
            } else {
                debuggerLibFile = new File('/debug' + parts.tail);
                response.sendRedirect('/' + fuseState.appName + '/debug/enable');
            }
            handled = true;
        } else if (parts.head == 'public') { // {one} == 'public'
