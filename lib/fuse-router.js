...
(function () {
    ...
    var renderPage = function (path) {
        var layout = null;
        var mainUnit = null;
        var unitDefinitions = fuse.getUnitDefinitions();
        var matched = [];
        for (var i = 0; i < unitDefinitions.length; i++) {
            var definition = unitDefinitions[i];
            if (fuse.isMatched(definition)) {
                matched.push(definition);
                var ancestors = fuse.getAncestors(definition.name);
                for (var k = 0; k < ancestors.length; k++) {
                    var ancestor = ancestors[k];
                    var zoneDef = fuse.getZoneDefinition(ancestor);
                    for (var j = 0; j < zoneDef.zones.length; j++) {
                        var zone = zoneDef.zones[j];
                        if (!fuseState.zones[zone]) {
                            fuseState.zones[zone] = [];
                        }
                        fuseState.zones[zone].push(ancestor);
                    }
                }
                ...
                if (zoneDef.layout) {
                    if (layout == null) {
                        ...
                        log.warn(
                            ...
                            layout + ' vs ' + definition.name + ':' + zoneDef.layout
                        );
                    }
                }
            }
        ...
        if (layout !== null) {
            for (zone in fuseState.zones) {
                fuse.cleanupAncestors(fuseState.zones[zone]);
            }

            log.debug(
                '[' + requestId + '] request for "' + path + '" will be rendered using layout "' +
                layout + '" (defined in "' + mainUnit + '") and zones ' +
                stringify(fuseState.zones));
            ...
            log.debug(
                ...
                'in any of the zones ' + stringify(fuseState.zones));
            return false;
        }
    };
