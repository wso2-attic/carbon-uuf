function StaticRouter() {
    ...
    function getFile(unitFullName, resourceType, fileName, lookUpTable, configs) {
        ...
        if (!unitModel) {
            log.warn(" Requested unit '" + unitFullName + "' does not exists.");
        ...
        log.warn(" Requested resource '" + fileName + "' does not exists in units ['" + unitFullName
                 + "', '" + parentUnitFullName + "'].");
    ...
    this.route = function (request, response, lookUpTable, configs) {
        ...
        if (parts.length == 5) {
            // parts = ["", {appName}, "public", {unitFullName}, {fileName}.{extension}]
            // an invalid URI
            log.warn("Request URI '" + uri + "' is invalid. "
